...
namespace UmbraInjector
{
    public partial class SearchingForProcessForm : Form
    {
        ...
        {
            string dllName = "";
            var currentFiles = Directory.GetFiles("Data/");
            foreach (string fileName in currentFiles)
            {
                string temp = fileName.Replace("Data/", "");
        ...
        public static void Inject(bool alreadyOpen)
        {
            ...
            startInfo.Arguments = $"/C cd Data&smi.exe inject -p \"Risk of Rain 2\" -a {GetDLLName()} -n UmbraUnlockAll -c Loader -m Load";
